		...
		function populateWithdraw() {
			$.ajax({  
				...
				success: function(dataString) {  
					...
					$('#amountinputfinal').value(parseFloat(document.getElementById('amountinput').value).toPrecision(2) + (parseFloat(dataString[8][parseInt(dataString[0])]) * parseFloat(document.getElementById('amountinput').value).toPrecision(2)));
