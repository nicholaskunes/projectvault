		...
		function populateWithdraw() {
			$.ajax({  
				...
				success: function(dataString) {  
					...
					$('#amountinputfinal').val(Number.toFixed(parseFloat(parseFloat(document.getElementById('amountinput').value).toPrecision(2) + (parseFloat(dataString[8][parseInt(dataString[0])]) * parseFloat(document.getElementById('amountinput').value).toPrecision(2))).toPrecision(2)));
